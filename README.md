...
foreach (var (source, valuables) in ContentRegistry.GetAllValuables())
{
...
Finding the source of an item:

```cs
// Item item = ...
var source = REPOLib.Modules.ContentRegistry.GetSource(item);
Debug.Log($"{item.name} comes from {source?.Name}");
```
